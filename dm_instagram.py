...
def parseXMLforKeyword(searchpath,keyword,exclude=""):
    myfiles = []
    ...
    excludes = str.split(exclude, " ")
    print "Excluding files whose names include: "+exclude
...
#parseXMLwithFunction("h:/data/rawdata/gnip/instagram/2014/07/25/","selfie",hasSelfieSpecific)
def parseXMLwithFunction(searchpath,keyword,function,exclude=""):
    myfiles = []
    for (dirpath, dirnames, filenames) in os.walk(searchpath):
        myfiles.extend(os.path.join(dirpath, filename) for filename in filenames)
...
def parseXMLrandom(searchpath,keyword,range,exclude=""):
    myfiles = []
    for (dirpath, dirnames, filenames) in os.walk(searchpath):
        myfiles.extend(os.path.join(dirpath, filename) for filename in filenames)
...
#findSelfies("h:/data/rawdata/gnip/instagram/2014/")
def findSelfies(mydir):
    parseXMLwithFunction(mydir,"selfie",hasSelfieSpecific,"selfie percent finishit eraseandreplace truthorange")
    parseXMLfileToCSV(mydir+"selfie.xml")
    getBigPictures(mydir+"selfie.xml.csv", "h:/data/rawdata/gnip/instagram/selfies/")

#findKeyword("h:/data/rawdata/gnip/instagram/2014/07/25", "finishit")
def findKeyword(mydir, keyword):
    parseXMLforKeyword(mydir,keyword,"selfie percent finishit eraseandreplace truthorange")
    parseXMLfileToCSV(mydir+keyword+".xml")
    getBigPictures(mydir+keyword+".xml.csv", "h:/data/rawdata/gnip/instagram/"+keyword+"/")
    
def sample(range):
    num = random.randint(1, range)
    return(num == range/2)

def getSample(mydir, range):
    p = str(100/range)
    parseXMLrandom(mydir,p+"percent",range,"selfie percent finishit eraseandreplace truthorange")
    parseXMLfileToCSV(mydir+p+"percent.xml")
    #getBigPictures(mydir+p+"percent.xml.csv", "h:/data/rawdata/gnip/instagram/"+p+"percent/")
